...
export function SequenceEditor({
  ...
  const [steps, setSteps] = useState<Step[]>(
    initialSteps?.length ? initialSteps : [{ subject: '', body: '', delayDays: 0 }]
  )
  ...
  const currentStep = steps[activeStep] ?? { subject: '', body: '', delayDays: 0 }
  const vars = makeVars(previewLead)
  const renderedSubject = renderVariables(expandSpintax(currentStep.subject), vars)
  const renderedBody = renderVariables(expandSpintax(currentStep.body), vars)

  function addStep() {
    setSteps((prev) => [...prev, { subject: '', body: '', delayDays: 1 }])
  ...
  function updateStep(field: keyof Step, value: string | number) {
    setSteps((prev) =>
